...
void GetEnergyResolutionProcessor::init()
{
	...
	_evHitsHist = new TH1D("_evHitsHist","Number of hits Distribution", 31, -5, 305);
	...
	for (int i = 0; i < 15; i++)
	{
		...
		_hitsInLayer[i] = new TH1D(Form("_HitsInLayer_%d",i+1),"Hits in layer ",11, -5, 105);
...
void GetEnergyResolutionProcessor::processEvent(LCEvent *evt)
{
	...
	{
			_evHitsHist->Fit("gaus");	//Fit a landau to the distribution
			...
			_evEnergyHist->Fit("gaus");	//Fit a landau to the distribution
